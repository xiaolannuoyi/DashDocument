...
let docset = {
    name: `VueJS.docset`,
    path: path.join(__dirname, `docset/VueJS.docset`),
    resPath: path.join(__dirname, `docset/VueJS.docset/Contents/Resources`),
    include: ['api','guide']
};
...
function readDirSync(path,dirName){
    let pa = fs.readdirSync(path); //读取目录
    pa.forEach(function(ele){
        let info = fs.statSync(path+"/"+ele);//我们往往通过该方法(statSync)，判断正在处理的到底是一个文件，还是一个目录
        if(info.isDirectory()){
